...
def mock_templates(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "cognithor.crew.cli.list_templates_cmd.TEMPLATES_ROOT", tpl_root
    )
...
def test_creates_project_from_template(tmp_path: Path, mock_templates):
    ...
    rc = run_init(
        name="My Project", template="research", directory=project_dir, lang="en"
    )
    assert rc == 0
...
def test_refuses_nonempty_directory(tmp_path: Path, mock_templates):
    ...
    with pytest.raises(InitCommandError):
        run_init(
            name="existing", template="research", directory=project_dir, lang="en"
        )
...
def test_init_force_overwrites_existing_dir(
    tmp_path: Path, mock_templates, capsys
):
